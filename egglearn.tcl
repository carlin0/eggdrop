...
#Comments, impressions and suggestions are welcome, send it to defcon7@twlc.net
#		   ./twlc group, http://www.twlc.net



#This program is distribuited under the terms of the artistic license, you got one copy of that in this package.
#For instructions read the README file
...
proc dbbck {nick uhost hand chan arg } {
        ...
        putquick "privmsg $chan :Database backup executed"
}
...
proc reqs_analisys_add { nick uhost hand chan arg } {
        ...
        if {$eglrn(addop) == "1"} {
                if {[isop $nick $chan] == 1} {
                       ...
                       if {[string match {*\**} $arg]} {
		       		putquick "NOTICE $nick :hmm you want add the world ? NoNoNo..."
		       		return
...
proc add_record { nick uhost hand chan arg } {
        ...
        if {[string length [lrange $arg 0 end]] > $eglrn(maxlength) } {
                putquick "NOTICE $nick :Warning the max length of a record is setted to $eglrn(maxlength) chars, contact the bot admin to increase that or add more short records"
                return
        }
        ...
        if { ($query != "") && ($definition != "")} {
                ...
                while {![eof $chid]} {
                        ...
                        if {$line == ""} {
                                ...
                                if {$first == $query} {
                                        putquick "NOTICE $nick :Warning that record is already in the database use $eglrn(modify) or $eglrn(rem) ($eglrn(hlp) for help)"
                                        close $chid
                                        set recaad 1
                                        break
                                } else {
   ...
   } else { putquick "NOTICE $nick :Sintassi: $eglrn(add) query definizione ($eglrn(hlp) per l'aiuto)"}
...
proc define_object { nick uhost hand chan arg } {
        ...
        set argline [string trimright [ string trimleft [string tolower [lrange $arg 0 end]]]]
                ...
                set definitions "0"
                        ...
                        } else {putquick "NOTICE $nick :Sintassi: $eglrn(define) query ($eglrn(hlp) per l'aiuto)"}
}
...
proc rem_record { nick uhost hand chan arg } {
        ...
        set query [string tolower [lindex $arg 0]]
                ...
                } else {putquick "NOTICE $nick :Sintassi: $eglrn(rem) query ($eglrn(hlp) per l'aiuto)"}
}
...
proc modify_record { nick uhost hand chan arg } {
        ...
        set definition [lrange $arg 1 end]
                ...
                } else { putquick "NOTICE $nick :Sintassi: $eglrn(modify) query definizione ($eglrn(hlp) per l'aiuto)"}
