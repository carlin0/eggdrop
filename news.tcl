...
#
#set feeds(punto-informatico:##) {90 http://punto-informatico.it/fader/pixml.xml}

#set feeds(mtv:##) {30 http://feeds.feedburner.com/mtvitalia_news}
#set feeds(alice:##) {90 http://musica.alice.it/news.xml}
#set feeds(sport:#gente) {37 http://www.gazzetta.it/rss/home.xml}
#set feeds(informatica:#amici) {111 http://feeds.punto-informatico.it/c/32288/f/438866/index.rss}
#set feeds(musica:#amici) {71 http://feeds.feedburner.com/rockol/fRwb?format=xml}

#set feeds(stampa-torino:#torino) {16 http://www.lastampa.it/torino/rss_home.xml}
#set feeds(google-torino:#torino) {20 http://news.google.it/news?q=torino&ie=UTF-8&hl=it&output=rss}

#set feeds(rainews:##italia) {43 http://www.servizitelevideo.rai.it/televideo/pub/rss102.xml}
set feeds(tgcom24:#amici) {43 http://www.tgcom24.mediaset.it/rss/homepage.xml}

#set feeds(corriere:#gente) {33 http://xml.corriereobjects.it/rss/homepage.xml}
#set feeds(repubblica:#gente) {39 http://data.kataweb.it/rss/news24repubblica/rss2.0.xml}

#set feeds(generali:#gente) {35 http://www.ansa.it/sito/ansait_rss.xml}
#set feeds(ultime:#gente) {22 http://www.ansa.it/sito/notizie/topnews/topnews_rss.xml}
#set feeds(cronaca:#gente) {55 http://www.ansa.it/sito/notizie/cronaca/cronaca_rss.xml}
#set feeds(Ansa-Tecnologia:#gente) {77 http://www.ansa.it/sito/notizie/tecnologia/tecnologia_rss.xml}
...
proc process {data chanfeed} {
	...
	foreach {foo item} [regexp -all -inline {(?i)<item.*?>(.*?)</item>} $data] {
		...
		if {[lsearch -exact $hash($chanfeed) [md5 $title]] == -1 && [botonchan $chan]} {
			if {![info exists header]} {
				...
				puthelp "privmsg $chan :\002Ultimissime news\002 da $header!"
			}
			if {$count < $maxnew} {
	...
	if {[info exists indices] && [botonchan $chan]} {
		...
		puthelp "privmsg $chan :...e altre $count $indices" 
	}
...
proc news {nick uhost hand chan text} {
	...
	if {$text == ""} {
		...
		if {[info exists names]} {
			...
			puthelp "notice $nick :rubriche per $chan: $names"
			puthelp "notice $nick :scrivi $::lastbind <rubrica> \[index#\]"
		} {
			puthelp "notice $nick :nessuna rubrica per $chan"
		}
		return 1
	}
	if {![info exists news($feed:$chan)]} {
		puthelp "notice $nick :nessuna news da $feed per $chan"
		return 1
	}
	if {$num == ""} {
		set idx 1
		...
		puthelp "notice $nick :Fonte News: \002$title\002"
	...
	if {$num < 1 || $num > [llength $news($feed:$chan)]} {
		...
		set idx [expr {$num-1}]
		puthelp "notice $nick :......titolo($num): [lindex [lindex $news($feed:$chan) $idx] 0]"
		puthelp "notice $nick :....articolo($num): [lindex [lindex $news($feed:$chan) $idx] 2]"
