...
void tell_verbose_status(int idx)
{
  ...
  dprintf(idx, "Io sono %s, eseguendo %s: %d user%s (mem: %uk).\n",
  ...
  dprintf(idx, "Configurato con: " EGG_AC_ARGS "\n");
  if (admin[0])
    dprintf(idx, "Amministratore: %s\n", admin);

  dprintf(idx, "File di configurazione: %s\n", configfile);
  sysrel = egg_uname();
  if (*sysrel)
    dprintf(idx, "OS: %s\n", sysrel);
  dprintf(idx, "Processo ID: %d (parent %d)\n", getpid(), getppid());
...
#ifdef TLS
  dprintf(idx, "Il supporto TLS è abilitato.\n"
...
#else
  dprintf(idx, "Il supporto TLS non è disponibile.\n");
#endif
#ifdef IPV6
  dprintf(idx, "Il supporto IPv6 è abilitato.\n"
#else
  dprintf(idx, "Il supporto IPv6 non è disponibile.\n"
#endif
#ifdef EGG_TDNS
...
void tell_settings(int idx)
{
  ...
  dprintf(idx, "Motd: %s\n",  motdfile);
  dprintf(idx, "Directori:\n");
#ifndef STATIC
  dprintf(idx, "  Help   : %s\n", helpdir);
  dprintf(idx, "  Moduli : %s\n", moddir);
#else
  dprintf(idx, "  Help: %s\n", helpdir);
#endif
