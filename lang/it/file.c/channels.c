...
static void write_channels()
{
  ...
  if (f == NULL) {
    putlog(LOG_MISC, "*", "ERRORE nello scrivere il file dei canali.");
    return;
  }
  if (!quiet_save)
    putlog(LOG_MISC, "*", "Scrivo il file dei canali...");
...
static void read_channels(int create, int reload)
{
  ...
  if (!readtclprog(chanfile) && create) {
    ...
    putlog(LOG_MISC, "*", "Creo il file dei canali");
    f = fopen(chanfile, "w");
    if (!f)
      putlog(LOG_MISC, "*", "Impossibile creare il file dei canali: %s.  Rinuncio",
             chanfile);
    else
      fclose(f);
  }
...
static void backup_chanfile()
{
  ...
  if (quiet_save < 2)
    putlog(LOG_MISC, "*", "Faccio una copia del file dei canali...");
