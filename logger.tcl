...
proc logger:time {} {
  ...
  switch [string toupper $logger(time)] {
    DAY {
      bind time - "00 00 [strftime "%e" [expr [unixtime] + 86400]] * *" logger:time-save
    }
    WEEK {
      bind time - "00 00 [strftime "%e" [expr [unixtime] + ((7 - [strftime "%w"]) * 86400)]] * *" logger:time-save
    }
    MONTH {
      bind time - "00 00 01 [strftime "%m" [expr [unixtime] + ((32 - [strftime "%e"]) * 86400)]] *" logger:time-save
    }
  }
}
